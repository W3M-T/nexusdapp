...
import ViewImagePopup, {
  ItemProps,
} from "../../shared/components/ui/CreationPop";
import { ChildLoader } from "../../shared/components/ui/Loader";
import Swal from "sweetalert2";
import { useGetLoginInfo } from "@multiversx/sdk-dapp/hooks/account/useGetLoginInfo";
import { useAllUsers } from "../../shared/hooks/auth/user";
...
function CommunityGallery() {
  ...
  const getData = useCallback(async () => {
    ...
    try {
      ...
      const imagesgallery = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      imagesgallery.sort(
        (a: imageProps, b: imageProps) => b.likes.length - a.likes.length
      );
      ...
      setImagesData((prev) =>
        prev.map((img) => {
          if (img.id === item.id) {
            return {
              ...img,
              likes: img.likes.filter(
                (like) => like.likeAddress !== account.address
              ),
            };
          }
          return img;
  ...
  return (
    <div>
      <div>
        <h1 className="flex justify-center items-center text-center text-3xl text-white font-bold mb-[2px]">
          Community Gallery
        </h1>
        <span className="flex text-[20px] font-medium text-white justify-center items-center text-center">
          Discover the best AI genrated artworks made using Pensy AI
          <br />
        </span>
      </div>
                ...
                {hoveredIndex === index && (
                  <div className="absolute bottom-0 left-0 flex items-center space-x-2 p-2">
                    <span style={{ display: "flex", width: "100%" }}>
                      {users.filter(val => val.walletAddress === item.walletAddress)[0]?.profileImage ? (
                        <Avatar
                          src={users.filter(val => val.walletAddress === item.walletAddress)[0]?.profileImage}
                          size={40}
                          className="!bg-grey-solid !rounded-full"
                        />
                      ) : (
                    ...
                    {item?.likes?.find(
                      (item) => item?.likeAddress == account.address
                    ) ? (
                      <div
                        className="bg-white rounded-full p-2 cursor-pointer hover:bg-gray-200"
